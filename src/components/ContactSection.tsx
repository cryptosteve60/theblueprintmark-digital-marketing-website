
import { Link } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Rocket, Users, Award } from "lucide-react";

const stats = [{
  icon: Users,
  value: "500+",
  label: "Happy Clients"
}, {
  icon: Rocket,
  value: "1000+",
  label: "Campaigns Launched"
}, {
  icon: Award,
  value: "95%",
  label: "Client Retention Rate"
}];

const ContactSection = () => {
  return (
    <section className="py-16 px-4">
      <div className="container mx-auto">
        {/* Stats Section */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {stats.map((stat, index) => (
            <div key={index} className="text-center">
              <div className="card-gradient w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 hover:shadow-glow-primary transition-all duration-300">
                <stat.icon className="h-6 w-6 text-primary" />
              </div>
              <div className="text-2xl font-bold mb-1 hero-text-glow">{stat.value}</div>
              <div className="text-muted-foreground text-sm">{stat.label}</div>
            </div>
          ))}
        </div>

        <div className="text-center mb-12">
          <Badge variant="outline" className="mb-3 border-accent text-accent text-xs">
            Get In Touch
          </Badge>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            Ready to
            <span className="tech-gradient bg-clip-text text-transparent"> Transform Your Business</span>?
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Let's discuss how we can accelerate your digital growth. Get a free consultation 
            and custom strategy for your business.
          </p>
        </div>
        
        {/* Centered Free Strategy Session Card */}
        <div className="flex justify-center mb-12">
          <Card className="card-gradient border-border/50 max-w-md w-full">
            <CardHeader className="pb-4">
              <CardTitle className="text-xl font-bold text-center">
                Free Strategy Session
              </CardTitle>
              <CardDescription className="text-sm text-center">
                Get a personalized digital marketing strategy tailored to your business goals.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="p-3 bg-primary/5 rounded-lg border border-primary/20">
                <h4 className="font-semibold text-primary mb-2 text-sm">What You'll Get:</h4>
                <ul className="space-y-1 text-xs">
                  <li className="flex items-center">
                    <div className="w-1 h-1 rounded-full bg-primary mr-2"></div>
                    Comprehensive business analysis
                  </li>
                  <li className="flex items-center">
                    <div className="w-1 h-1 rounded-full bg-primary mr-2"></div>
                    Custom digital marketing strategy
                  </li>
                  <li className="flex items-center">
                    <div className="w-1 h-1 rounded-full bg-primary mr-2"></div>
                    ROI projections and timeline
                  </li>
                  <li className="flex items-center">
                    <div className="w-1 h-1 rounded-full bg-primary mr-2"></div>
                    Competitive analysis report
                  </li>
                </ul>
              </div>
              
              <div className="space-y-2">
                <Button variant="hero" className="w-full py-4" asChild>
                  <Link 
                    to="/consultation"
                    aria-label="Book a free strategy consultation to discuss your digital marketing goals"
                  >
                    Book Free Consultation
                    <Rocket className="ml-2 h-4 w-4" aria-hidden="true" />
                  </Link>
                </Button>
                
                <Button variant="outline" className="w-full text-sm py-2" asChild>
                  <Link 
                    to="/case-studies"
                    aria-label="Download detailed case studies showing our client success stories"
                  >
                    Download Our Case Studies
                  </Link>
                </Button>
              </div>
              
              <div className="text-center">
                <p className="text-xs text-muted-foreground">
                  🔒 No spam, no commitments. Just valuable insights for your business.
                </p>
              </div>
            </CardContent>
          </Card>
        </div>
        
        {/* Bottom CTA */}
        <div className="text-center p-6 card-gradient rounded-xl border border-primary/20">
          <h3 className="text-xl font-bold mb-3">
            Ready to Dominate Your Digital Space?
          </h3>
          <p className="text-muted-foreground mb-4 max-w-xl mx-auto text-sm">
            Join hundreds of successful businesses that have transformed their digital presence with our proven strategies.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button variant="cta" size="lg" asChild>
              <Link to="/consultation">Start Your Journey</Link>
            </Button>
            <Button variant="tech" size="lg" asChild>
              <Link to="/case-studies">View Success Stories</Link>
            </Button>
          </div>
        </div>
      </div>
    </section>
  );
};

export default ContactSection;
